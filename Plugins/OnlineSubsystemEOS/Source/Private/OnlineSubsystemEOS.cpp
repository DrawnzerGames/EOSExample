...
void EOS_CALL EOSLog(const EOS_LogMessage* InMsg)
{
    ...
    const FString AndroidMessage = FString::Printf(TEXT("EOSSDK-%s: %s"), ANSI_TO_TCHAR(InMsg->Category), ANSI_TO_TCHAR(InMsg->Message));
    if(GEngine)
    {
        //GEngine->AddOnScreenDebugMessage(FMath::Rand(), 10.0f, FColor::Emerald, *AndroidMessage);        
    }
    switch (InMsg->Level)
    {
...
void FOnlineSubsystemEOS::LoadDlls()
{
    const FString BaseDir = IPluginManager::Get().FindPlugin("OnlineSubsystemEOS")->GetBaseDir();
    // Add on the relative location of the third party dll and load it
    FString LibraryPath;
#if PLATFORM_WINDOWS
#if PLATFORM_32BITS
    LibraryPath = FPaths::Combine(*BaseDir, TEXT("EOS/EOS-SDK-1.8/SDK/Bin/EOSSDK-Win32-Shipping.dll"));
#else
    LibraryPath = FPaths::Combine(*BaseDir, TEXT("EOS/EOS-SDK-1.8/SDK/Bin/EOSSDK-Win64-Shipping.dll"));
#endif // PLATFORM_WINDOWS

#elif PLATFORM_MAC
    LibraryPath = FPaths::Combine(*BaseDir, TEXT("EOS/EOS-SDK-1.8/SDK/Bin/libEOSSDK-Mac-Shipping.dylib"));
#endif

#if PLATFORM_LINUX
    LibraryPath = FPaths::Combine(*BaseDir, TEXT("EOS/EOS-SDK-1.8/SDK/Bin/libEOSSDK-Linux-Shipping.so"));
#endif
...
void FOnlineSubsystemEOS::ModuleInit()
{
    ...
    {
        EOS_Android_InitializeOptions JNIOptions = { 0 };
        JNIOptions.ApiVersion = EOS_ANDROID_INITIALIZEOPTIONS_API_LATEST;
        JavaVM* VM = nullptr;
        Env->GetJavaVM(&VM);
        JNIOptions.VM = VM;

        //Todo pull android specific package name at runtime 
        const FString Internal = "/data/user/0/com.mycompany.EOS/files/";
        const FString External = "/storage/emulated/0/Android/data/com.mycompany.EOS/files/";
        int Len = Internal.Len();
        int ExtLen = External.Len();
        char InternalDir[Len];
        char ExternalDir[ExtLen];
        FCStringAnsi::Strncpy(InternalDir, TCHAR_TO_UTF8(*Internal), Len);
        FCStringAnsi::Strncpy(ExternalDir, TCHAR_TO_UTF8(*External), ExtLen);
        
        JNIOptions.OptionalInternalDirectory = InternalDir;
        JNIOptions.OptionalExternalDirectory = ExternalDir;
        SDKOptions.SystemInitializeOptions = &JNIOptions;
    }else
    {
        UE_LOG_ONLINE(Error, TEXT("EOS JNI Init Failed."));
    }
#endif
...
{
    const FString ClientId = IsRunningDedicatedServer() ? "" : "";
    const FString ClientSecret = IsRunningDedicatedServer() ? "" : "";
    ...
    */
...
#if PLATFORM_ANDROID
    //Todo pull android specific cache dir at runtime 
    CacheDir = TEXT("/data/user/0/com.mycompany.EOS/cache");
#endif
    ...
    UE_LOG_ONLINE(Log, TEXT("FOnlineSubsystemEOS: Create without any errors."));
    return true;
}
